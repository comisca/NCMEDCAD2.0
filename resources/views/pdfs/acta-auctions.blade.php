...
<body>
@if($actaData->auction_result == 'Adjudicado')
    <div class="header">
    ...
    <div class="info-box">
        ...
        <span class="text-bold">Precio de referencia:</span>
        ${{ number_format($actaData->price_reference, 4, '.', ',') }}
        <br>
    </div>
    ...
    <p>Habiendo finalizado con el proceso de negociación bajo la modalidad de SUBASTA A LA {{$actaData->type_auction}} y
        obteniendo un
        precio
        favorable a la región, se da por adjudicado a la empresa farmacéutica: {{$pujaWinner->legal_name}}, con un
        precio de
        USD:
        {{ number_format($pujaWinner->amount, 4, '.', ',') }}, el cual incluye: costo, seguro y flete; debiendo
        legalizarlo
    ...
    <p>Las cantidades de compra pueden modificarse a petición de cada una de las Instituciones de Salud participantes,
        de
        acuerdo a
        ...
        adjudicación se
        detallará en el acta de adjudicación del evento de negociación de precios, a partir de esa fecha se podrá
        proveer
        los bienes
        adjudicados y en los dos años siguientes al efectuado el evento de negociación de precios, con prórroga
        facultativa
        del precio
    ...
    <p>La empresa adjudicada se compromete a realizar las gestiones administrativas legales correspondientes con su
        representante o
        distribuidor local con el fin que se haga efectivo el precio negociado en la ejecución de las compras a nivel
        local
        por cada una de
...
@else
    ...
    <div class="result-box">
        Luego de finalizado el tiempo de la subasta, y no habiendo obtenido un precio favorable para la región,
        se da por cerrada la subasta, declarando el producto como NO RECOMENDADO.
    </div>
